...
"hasil_evaluation_ld_based_on_arch_popiter_ll.csv"
]
# folder='popiter'
# folder='f_cr'
# folder='wmanual'
...
def create_label_from_file(filename: str):
    ...
    if len(names) == 2 or len(names)==1:
        return names[-1]
    if "0." in filename:
        return names[6]+" "+names[7]+("WC" if "no_candidate" not in filename else " NC")+("" if "v2" not in filename else " v2")
    return names[3]+" "+names[4]+("WC" if "no_candidate" not in filename else " NC")+("" if "v2" not in filename else " v2")
...
plt.title('Comparison RMSE value Arch DE training (cr=0.7 & f=0.5)')
plt.tight_layout()
plt.show()
