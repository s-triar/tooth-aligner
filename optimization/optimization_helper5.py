...
from utility.calculation import find_closest_point_between_a_point_and_a_3pts_plane, is_point_in_triangle_3d, is_point_in_a_line
...
DISTANCE_MESIODISTAL_BONWILL_ERROR_WEIGHT = 1 # 1
...
def calculate_angle_bucal_labial_to_bonwill_belt_with_std(tooth, belts):
    # belts = custp, mid, gum -> udah diambil per arch
    points = tooth.vertices[tooth.buccal_labial_side_index]
    distances = []
    plane = np.array([
            belts[0][tooth.label][0], #cusp mesial,
            belts[0][tooth.label][1], #cusp distal
            belts[2][tooth.label][1]  #gum distal
        ])
    for p in points:
...
def calculate_distance_bucal_labial_to_bonwill_belt(tooth, belts):
    ...
    plane1 = np.array([
        belts[0][tooth.label][0],  # cusp mesial,
        belts[0][tooth.label][1],  # cusp distal
        belts[2][tooth.label][0]  # gum mesial
    ])
    rect = np.array([
        belts[0][tooth.label][0],  # cusp mesial,
        belts[0][tooth.label][1],  # cusp distal
    ...
    for p in points:
        g = find_closest_point_between_a_point_and_a_3pts_plane(p,plane)
        if (is_point_in_triangle_3d(g, plane)):
            distances.append(find_distance_between_two_points(p,g))
        elif( is_point_in_triangle_3d(g, plane1)):
            distances.append(find_distance_between_two_points(p, g))
        else:
            dd = []
            for j in rect:
                dd.append(find_distance_between_two_points(g, j))
...
def calculate_tooth_balance_to_bonwill_belt(tooth, belts, for_cr=False):
    # belts = custp, mid, gum -> udah diambil per arch dan per tooth
    if for_cr:
        return calculate_angle_bucal_labial_to_bonwill_belt_with_std(tooth,
                                                                     belts), calculate_distance_bucal_labial_to_bonwill_belt(
            tooth, belts)
    return calculate_angle_bucal_labial_to_bonwill_belt_with_std(tooth, belts) + calculate_distance_bucal_labial_to_bonwill_belt(tooth,belts)
